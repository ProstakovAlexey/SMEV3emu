import smev3


def send_response(text):
    """Всегда подтверждает ответ"""
    message_id = smev3.get_message_id(text)
    t = smev3.time_stamp()
    xml = """
    <soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
	<soap:Body>
		<ns2:SendResponseResponse xmlns="urn://x-artefacts-smev-gov-ru/services/message-exchange/types/basic/1.1" xmlns:ns2="urn://x-artefacts-smev-gov-ru/services/message-exchange/types/1.1" xmlns:ns3="urn://x-artefacts-smev-gov-ru/services/message-exchange/types/faults/1.1">
			<ns2:MessageMetadata Id="SIGNED_BY_SMEV">
				<ns2:MessageId>%s</ns2:MessageId>
				<ns2:MessageType>RESPONSE</ns2:MessageType>
				<ns2:Sender>
					<ns2:Mnemonic>021701_3T</ns2:Mnemonic>
					<ns2:HumanReadableName>АИС «АСП»</ns2:HumanReadableName>
				</ns2:Sender>
				<ns2:SendingTimestamp>%s</ns2:SendingTimestamp>
				<ns2:DestinationName/>
				<ns2:Recipient>
					<ns2:Mnemonic>MNSV05</ns2:Mnemonic>
					<ns2:HumanReadableName>ЕПГУ (svcdev)</ns2:HumanReadableName>
				</ns2:Recipient>
				<ns2:SupplementaryData>
					<ns2:DetectedContentTypeName>not detected</ns2:DetectedContentTypeName>
					<ns2:InteractionType>NotDetected</ns2:InteractionType>
				</ns2:SupplementaryData>
				<ns2:Status>requestIsQueued</ns2:Status>
			</ns2:MessageMetadata>
			<ns2:SMEVSignature>
				<ds:Signature xmlns:ds="http://www.w3.org/2000/09/xmldsig#">
					<ds:SignedInfo>
						<ds:CanonicalizationMethod Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#"/>
						<ds:SignatureMethod Algorithm="http://www.w3.org/2001/04/xmldsig-more#gostr34102001-gostr3411"/>
						<ds:Reference URI="#SIGNED_BY_SMEV">
							<ds:Transforms>
								<ds:Transform Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#"/>
								<ds:Transform Algorithm="urn://smev-gov-ru/xmldsig/transform"/>
							</ds:Transforms>
							<ds:DigestMethod Algorithm="http://www.w3.org/2001/04/xmldsig-more#gostr3411"/>
							<ds:DigestValue>+SIYvf3UH7Sug01mxqDoVgObfU3T7CyBzTu5RVGZXa4=</ds:DigestValue>
						</ds:Reference>
					</ds:SignedInfo>
					<ds:SignatureValue>dF/CWfTztenxoxmQ/sgjkv+Rzazpy8MehgZ4LpVDvBMeslN2nYgC6t3yiFXpwObpiKbD2XwUCXLQCx2VP9GIRQ==</ds:SignatureValue>
					<ds:KeyInfo>
						<ds:X509Data>
							<ds:X509Certificate></ds:X509Certificate>
						</ds:X509Data>
					</ds:KeyInfo>
				</ds:Signature>
			</ns2:SMEVSignature>
		</ns2:SendResponseResponse>
	</soap:Body>
</soap:Envelope>
    """ % (message_id, t)
    return xml