...
свой IP адрес.
2. На ТИ  в задаче `Сервис -> Настройка СМЭВ 3` укажите URL адрес WSDL как 
...
-----------------------
Эмулятор может отправлять запросы на ТИ. Если вы хотите, чтобы запросы отправились по одному разу, то
скопируйте запрос (запросы) в папку `Запросы к нам`. Если нужно, чтобы отправлялись постоянно, то в 
папку `Шаблоны\Запросы к нам` - т.о. вы можете загрузить несколько тысяч запросов: например создали 10 шаблонов, 
ТИ настроили на опрос раз в 5 сек, в минуту будет загружено 10*12=120 запросов.  
...
9. Предоставление сведений о наличии (отсутствии) задолженности по уплате налогов, сборов, страховых взносов, пеней, 
штрафов, процентов (VS00026v001-FNS001)
10. Ответ на заявление по детским.

Подстановка 
-----------
Чтобы запросы выглядели аккуратно (текущая дата, уникальный messageId), можно в них применить шаблоны подстановки:
1. **{MessageID}** - уникальный uuid, по стандарту СМЭВ-3;
2. **{TransactionCode}** - тоже уникальный uuid;
3. **{Timestamp}** - текущее время;
4. **{orderId}** - уникальный номер заявления.

В ответах подстановки используются автоматически - время и messageId. 


Генерация заявлений
-------------------
Для проверки настроек ТИ по работе с ЕФТТ можно использовать следующее: на основе справочников ЧТЗ создать заявления,
чтобы их отправить на ТИ. Т.о. будут проверены настройки ТИ и потом связывание в АСП.

Для генерации заявлений выполните:
1. В папку ``Шаблоны\Справочники`` скопировать справочники - слотов, вариантов оказания услуг, категорий и связка 
категорий и вариантов оказания услуг.
2. В файле ``make_jku_zaiv.py`` найти  ``Шаблоны/Заявления/Малоимущие.xml`` указать нужный шаблон.
3. Исправить ``change['oktmo'] = '05000000'`` на нужный.
4. Запустить ``make_jku_zaiv.py`` он создаст заявления в папку ``Запросы к нам``
